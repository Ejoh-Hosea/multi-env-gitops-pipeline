...
//
// Endpoints:
//   GET  /            -> basic greetings + environment + version info
...
app.get("/", (req, res) => {
  if (FAILURE_RATE > 0 && Math.random() < FAILURE_RATE) {
    return res
      .status(500)
      .json({ error: "injected failure for canary testing" });
  }
  res.json({
...
app.get("/version", (req, res) =>
  res.json({ version: APP_VERSION, gitSha: GIT_SHA, environment: ENVIRONMENT }),
);
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(
      `app listening on :${PORT} | env=${ENVIRONMENT} | version=${APP_VERSION} | gitSha=${GIT_SHA}`,
    );
  });
}
